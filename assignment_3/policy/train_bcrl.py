...
class Workspace:
    ...
    def setup(self):
        # create logger
        self.logger_bc = Logger(self.work_dir, use_tb=self.cfg.use_tb, mode="bc")
        self.logger_rl = Logger(self.work_dir, use_tb=self.cfg.use_tb, mode="rl")
        # create envs
    ...
    def eval(self, mode):
        ...
        logger = self.logger_bc if mode == "bc" else self.logger_rl
        with logger.log_and_dump_ctx(self.global_frame, ty="eval") as log:
    ...
    def train_bc(self):
        ...
        # predicates
        train_until_step = utils.Until(self.cfg.num_train_frames_bc, 1)
        eval_every_step = utils.Every(self.cfg.suite.eval_every_frames, 1)
        ...
        while train_until_step(self.global_step):
            ...
            # Update
            metrics = self.agent.update_bc(
                self.expert_replay_iter,
                self.global_step,
            )
            self.logger_bc.log_metrics(metrics, self.global_frame, ty="train")

            # log
            # if log_every_step(self.global_step):
            if self.global_step % 100 == 0:
                elapsed_time, total_time = self.timer.reset()
                with self.logger_bc.log_and_dump_ctx(
                ...
                if self.cfg.suite.save_snapshot:
                    self.save_snapshot(mode="bc")

            self._global_step += 1

        # Renames files to avoid overwriting
        shutil.move(self.work_dir / "train.csv", self.work_dir / "train_bc.csv")
        shutil.move(self.work_dir / "eval.csv", self.work_dir / "eval_bc.csv")
        shutil.move(self.work_dir / "eval_video", self.work_dir / "eval_video_bc")

        self._global_step = 0
        self._global_episode = 0

        print("Completed BC training. Starting RL training...")
    ...
    def train_rl(self):
        ...
        while train_until_step(self.global_step):
            ...
            if not seed_until_step(self.global_step):
                # Update
                metrics = self.agent.update_rl(
                    self.replay_iter,
                    self.expert_replay_iter,
                    self.global_step,
                )
                self.logger_rl.log_metrics(metrics, self.global_frame, ty="train")
    ...
    def save_snapshot(self, mode):
        snapshot = self.work_dir / f"snapshot_{mode}.pt"
...
def main(cfg):
    from train_bcrl import Workspace as W
